...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.tutorial3.rev191119.access.list.access.list.entry.actions.PacketHandling;
...
public class FlowData {
    ...
    public FlowData(AccessListEntry entry) {
        ...
        this.etherType = Tutorial3AppConstants.ETH_TYPE ;
        // TODO change the priority accordingly
        this.priority = Tutorial3AppConstants.FLOW_PRIORITY ;
        this.hardTimeout = Tutorial3AppConstants.DEFAULT_TIMEOUT_VALUE ;
        this.idleTimeout = Tutorial3AppConstants.DEFAULT_TIMEOUT_VALUE ;
        ...
        String [] parts = dstIpAddressTmp
                .split(Tutorial3AppConstants.SUBNET_MASK_SEPARATOR) ;
    ...
    public boolean equals(Object obj) {
        ...
        if (etherType != flowData.etherType) {
            return false;
        }
        if (priority != flowData.priority) {
            return false;
        }
        if (hardTimeout != flowData.hardTimeout) {
            return false;
        }
        if (idleTimeout != flowData.idleTimeout) {
            return false;
        }
        if (!dstIpAddress.equals(flowData.dstIpAddress)) {
            return false;
        }
        if (!nodeId.equals(flowData.nodeId)) {
            return false;
        }
